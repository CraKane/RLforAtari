...
parser.add_argument('--algorithm', type=int, default=3, metavar='N',
                    help='choose the algorithm you need (default: 1)(choice: [1, 2, 3])')
...
def final_policy():
    ...
    for t in range(10000):
        ...
        # print("aAAA", state)
        with torch.no_grad():
            action = point_get_action(agent.theta, state_var).numpy()
            action = action.astype(np.float64)

        next_state, reward, done, _ = env.step(action)
        statex.append(next_state[0])
        statey.append(next_state[1])
    ...
    plt.scatter(statex, statey, s=2, c="#66CCEE", marker='o')
    plt.scatter(statex[0], statey[0], s=10, c="#60F09A")
    plt.scatter(statex[-1], statey[-1], s=10, c="#60F09A")
