...
class QNetwork(nn.Module):
    def __init__(self):
        ...
        self.fc3 = nn.Linear(84, action_n+1)
    ...
    def dueling_value(self, network_value):
        state_value = network_value[:,-1]
        basic = torch.mean(network_value[:, :-1], dim=1)
        q_values = state_value.unsqueeze(1) + (network_value[:, :-1] - basic.unsqueeze(1))
        return q_values
...
def train(q, q_target, memory, optimizer):
    for i in range(10):
        ...
        q_out = q(s)
        q_values = q.dueling_value(q_out)
        ...
        max_q_primes = q_target.dueling_value(q_target_out)
